...
void /*...*/ PicReader::getData(Array& image) {
	HRESULT hr = S_OK;
	...
	******************************************************************/
	image.reshape(y, x, 4); //接收像素信息，按宽度、高度重塑
	for (int i = 0; i < y; i++) {
		for (int j = 0; j < x; j++) {
			for (int k = 0; k < 4; k++) {
				image.at(i, j, k).set(data[i * x * 4 + j * 4 + k]);
			}
		}
	}
	delete[] data; // 直接使用函数内申请的内存指针，故每次检查完数据后需要释放一次data
	/******************************************************************
	*  TO-DO END                                                      *
	******************************************************************/
