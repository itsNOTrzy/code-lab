...
void GameFrame::StartMenu()
{
	...
	outtextxy(700, 25, _T("By Eisenberg Andrew Roland"));   // 绘制游戏作者的名字
                       settextstyle(30, 0, _T("华文行楷"));    // 设置字体的大小
                       outtextxy(230, 85, _T("Welcome to Puzzle 2.0"));     // 绘制提示信息
	 settextstyle(23, 0, _T("华文行楷"));     // 设置字体信息
...
void Picture::IsPass()
{
	...
	settextstyle(23, 0, _T("华文行楷"));								// 设置字体信息
	outtextxy(700, 350, _T("游戏中"));						// 输出游戏状态
	...
	if (flag == Diffcult * Diffcult)
	{
		...
		loadimage(&img, String.c_str(), Width, Height);					// 加载图像
		putimage(0, 0, &img);
		outtextxy(700, 350, _T("已通关"));					// 输出游戏状态
		++Pass;															// 关数加一
