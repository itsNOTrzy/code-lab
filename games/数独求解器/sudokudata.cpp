...
void SudokuData::getNewPuzzle(int(*data)[9])
{
	...
	while (true)
	{
		row = rand() % 9;
		column = rand() % 9;
		if (inputData[row][column]) continue;
		num = rand() % 9 + 1;
		if (!isValidNum(inputData, row, column, num)) continue;
		inputData[row][column] = num;
...
bool SudokuData::dfsNum(int order, int &ans, int max, time_t beg, int time_limit)
{
	...
	for (int i = 1; i <= 9; i++)
	{
		if (!numCanTry[filledBoxs][row][column][i]) continue;
		fillAnswerNum(currentAnswerData, row, column, i, order);
		if (dfsNum(order + 1, ans, max, beg, time_limit)) return true;
...
bool SudokuData::dfsPrevious(int order)
{
	...
	for (int i = 9; i >= 1; i--)
	{
		...
		if (dfsPrevious(order + 1)) return true;
		unfillAnswerNum(previousAnswerData, row, column);
	}
	return false;
}
...
bool SudokuData::dfsNext(int order)
{
	...
	for (int i = 1; i <= 9; i++)
	{
		...
		if (dfsNext(order + 1)) return true;
		unfillAnswerNum(nextAnswerData, row, column);
	}
	return false;
}
