...
{
showText="Are you sure to exit Playback Mode?";
ExiRep=1;//指示已经按下过后移
}
}
...
void ReplayMode(){
int i,j;//循环变量
system("title Gobang AirLight Playback Mode");
showText="";//重置显示信息
count=0;//回合数归零
Putable=0;//不可走棋状态
...
while(1)//循环执行游戏，利用它达到“不退出程序”的效果
{
RunGame();
}
}
